...
| End-to-end asset → verify → scan → findings → risk → AI → report | **Covered** by `backend/tests/test_product_pipeline.py` (API + real orchestrator; HTTP/DNS/TLS/LLM mocked; includes ownership verification). Not Playwright. |

...
| End-to-end asset → verify → scan → findings → risk → AI → report | **Covered** by `backend/tests/test_product_pipeline.py` (inline/dev). Staging worker path + PDF after restart: `tests/test_staging_acceptance.py` (Docker; required CI job / `make staging-acceptance`). Not Playwright. |
...
| Backup/restore automation | **Implemented** — required CI quality-gate job (`make backup-integration-test`), `infrastructure/backup/`, `tests/test_backup_restore.py` |
| CI quality gate | **Implemented** — [deployment/ci.md](../deployment/ci.md) |
| Staging acceptance (workers + durable PDF + restart) | **Implemented** — required CI quality-gate job, `make staging-acceptance`, [deployment/staging.md](../deployment/staging.md) |

...
| `test_backup_restore.py` | Backup/restore Docker integration (`@pytest.mark.docker`; CI restore drill) |
...
| `test_staging_acceptance.py` | Staging Compose acceptance (workers, PDF after restart, audit; `@pytest.mark.docker`; required CI job) |

...
Naming: see [glossary.md](../glossary.md). Canonical actions are `{domain}.{verb}` (e.g. `asset.create`).
...
**Aliases:** publishers may still pass `ASSET_CREATED` or `entity_type` / `entity_id`; they are normalized to `asset.create` and stored as `resource_type` / `resource_id`. Prefer the canonical forms in new code and docs. See [event-catalog.md](./event-catalog.md).
...
| user_id | UUID |
| action | VARCHAR (canonical `asset.create`) |
...
**API aliases** (not DB columns):

| API query / response field | Maps to DB |
|----------------------------|------------|
...
Filters: `action` (prefer `scan.completed`; `SCAN_COMPLETED` is accepted and normalized), `severity`, `user_id`, `actor`, `asset_id`, `date_from`, `date_to`, `entity_type`, `entity_id`.
...
2. Prefer `event_bus.publish` with canonical `asset.create` and `resource_type` / `resource_id`
3. Include useful `details` (never passwords, tokens, or API keys)

...
| **FR-AUD-02** | The system shall store each audit row with organization, actor, resource (`resource_type` / `resource_id`), severity (`info`, `warning`, `error`, `critical`), details, IP, and user agent. | System | API aliases: `entity_type` → `resource_type`, `entity_id` → `resource_id`. See [glossary](../glossary.md). |
...
| **FR-AUD-05** | The system shall allow members with `org:read` to list, get, filter, and export audit logs as CSV or PDF. | Roles with `org:read` | Filters: action, severity, user, asset, date, `entity_type` / `entity_id` (aliases for `resource_*`). |

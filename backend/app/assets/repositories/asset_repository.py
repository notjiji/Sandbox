...
_SORT_COLUMNS = {
    ...
    AssetSortField.CRITICALITY: Asset.criticality,
    AssetSortField.ENVIRONMENT: Asset.environment,
    AssetSortField.TYPE: Asset.type,
}
...
def _resolve_order_by(
    *,
    ...
    if parent_id is not None or roots_only:
        column = _SORT_COLUMNS.get(sort, Asset.created_at)
        if order == SortOrder.DESC:
            return (column.desc(), Asset.id.asc())
        return (column.asc(), Asset.id.asc())
...
def list_child_assets(
    ...
    *,
    ...
    order_column = _SORT_COLUMNS.get(sort, Asset.created_at)
    if order == SortOrder.DESC:
        order_column = order_column.desc()
    else:
        order_column = order_column.asc()
    return query.order_by(order_column, Asset.id.asc()).all()

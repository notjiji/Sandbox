...
function ExpandButton({ expanded, disabled, onClick, label }) {
  return (
    <button
      type="button"
      disabled={disabled}
...
function AssetNameCell({
  ...
  const canExpand = mode === "tree" && depth === 0 && asset.children_count > 0;
      ...
      <div className={`flex items-start ${indent ?? ""}`}>
        ...
        <div className="min-w-0 flex-1">
          {child && (
            <CornerDownRight
              size={14}
              className="mb-1 inline-block text-brand-600"
              aria-hidden
            />
          )}
          <Link
          ...
          {mode === "tree" && depth === 0 && asset.children_count > 0 && !expanded && (
            <p className="mt-0.5 text-xs text-brand-600">
...
function AssetDataRow({
  ...
  return (
    <tr
      className={child ? "bg-brand-950/20 hover:bg-brand-900/30" : "hover:bg-brand-900/20"}
    >
      <td className="px-4 py-3">

...
export default function AssetForm({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAlert(err instanceof ApiError ? err.message : `Unable to ${isEdit ? "update" : "create"} asset.`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {needsOsFields && (
        <>
          <div>
            <label htmlFor="os" className="terminal-text mb-2 block">operating system</label>
            <input
              id="os"
              value={form.os}
          ...
          <div>
            <label htmlFor="connection_type" className="terminal-text mb-2 block">connection type</label>
            <select
              id="connection_type"
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {showStatusField ? (
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-brand-500">Use Restore on the detail page to change lifecycle status.</p>
          </div>
        )}
        <div>
      ...
      >
        ...
        {submitting ? (isEdit ? "Saving..." : "Adding...") : (submitLabel ?? (isEdit ? "Save changes" : "Add asset"))}
      </button>
    </motion.form>
  );
}

...
import { Bug, FileText, Globe, ShieldAlert } from "lucide-react";
import DashboardShell from "@/features/organizations/components/DashboardShell";
import FormAlert from "@/shared/components/FormAlert";
import { ApiError } from "@/shared/api/client";
import { riskApi } from "@/features/risk/api";
...
export default function ProjectDetail() {
  ...
  return (
    ...
    >
      ...
      <ProjectNav projectName={project?.name} active="risk" />
        ...
        <>
          {risk && (
            <motion.div
              ...
              className="glass-panel mb-6 flex flex-wrap items-center justify-between gap-4 p-6"
            >
              <div className="flex items-center gap-3">
                <ShieldAlert size={24} className="text-brand-400" />
                <div>
                  <p className="text-sm text-brand-500">Open-finding risk score</p>
                  <p className="text-2xl font-semibold text-brand-100">{risk.score.toFixed(2)}</p>
                </div>
              </div>
              <p className="text-sm text-brand-500">{risk.open_findings} open findings</p>
            </motion.div>
          )}
